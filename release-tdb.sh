...
##  3. clone the repo
git clone --branch=cata_classic $REPO_URL server
cd server
...
mkdir -p sql/old/$MAJOR_VERSION.$MINOR_VERSION.x/hotfixes/$OLD_TDB_FOLDER && mv sql/updates/hotfixes/cata_classic/* sql/old/$MAJOR_VERSION.$MINOR_VERSION.x/hotfixes/$OLD_TDB_FOLDER/
...
do
  if test -n "$(find ./sql/old/"$MAJOR_VERSION"."$MINOR_VERSION".x/auth/$OLD_TDB_FOLDER/ -maxdepth 1 -name "$TODAY"_$(printf "%02d" $counter)\*.sql -print -quit)"; then
    ...
    cat > sql/updates/auth/cata_classic/$TODAY\_$(printf "%02d" $counter)_auth.sql <<EOL
-- $NEW_TDB_NAME auth
...
do
  if test -n "$(find ./sql/old/"$MAJOR_VERSION"."$MINOR_VERSION".x/characters/$OLD_TDB_FOLDER/ -maxdepth 1 -name "$TODAY"_$(printf "%02d" $counter)\*.sql -print -quit)"; then
    ...
    cat > sql/updates/characters/cata_classic/$TODAY\_$(printf "%02d" $counter)_characters.sql <<EOL
...
do
  if test -n "$(find ./sql/old/"$MAJOR_VERSION"."$MINOR_VERSION".x/world/$OLD_TDB_FOLDER/ -maxdepth 1 -name "$TODAY"_$(printf "%02d" $counter)\*.sql -print -quit)"; then
    ...
    cat > sql/updates/world/cata_classic/$TODAY\_$(printf "%02d" $counter)_world.sql <<EOL
-- $NEW_TDB_NAME world
...
do
  if test -n "$(find ./sql/old/"$MAJOR_VERSION"."$MINOR_VERSION".x/hotfixes/$OLD_TDB_FOLDER/ -maxdepth 1 -name "$TODAY"_$(printf "%02d" $counter)\*.sql -print -quit)"; then
    ...
    cat > sql/updates/hotfixes/cata_classic/$TODAY\_$(printf "%02d" $counter)_hotfixes.sql <<EOL
...
mysqldump -uroot auth --default-character-set='utf8mb4' --hex-blob --routines --result-file sql/base/auth_database.sql
...
mysqldump -uroot characters --default-character-set='utf8mb4' --hex-blob --routines --result-file sql/base/characters_database.sql
...
sed -i -e 's/DEFINER=.* \(SQL SECURITY\)/\1/' sql/base/dev/hotfixes_database.sql
...
mkdir tdb
cd tdb
mysqldump -uroot world --default-character-set='utf8mb4' --hex-blob --routines --result-file $NEW_TDB_WORLD_FILE.sql
mysqldump -uroot hotfixes --default-character-set='utf8mb4' --hex-blob --routines --result-file $NEW_TDB_HOTFIXES_FILE.sql
...
sed -i -e 's$ #define DATABASE_HOTFIXES_DATABASE  "[A-Za-z0-9$_.]*"$ #define DATABASE_HOTFIXES_DATABASE  "'$NEW_TDB_HOTFIXES_FILE'.sql"$g' revision_data.h.in.cmake
...
cd tdb
NEW_RELEASE_RESPONSE=$(curl -s -H "Authorization: token $GITHUB_TOKEN" -d '{"tag_name":"'"$NEW_TDB_TAG"'","target_commitish":"cata_classic","name":"'"$NEW_TDB_NAME"'","body":"### ![cata_classic](https://img.shields.io/badge/branch-cata__classic-AA4203.svg)\n'"$NEW_TDB_RELEASE_NOTES"'\n","draft":true,"prerelease":false}' $GITHUB_API)
